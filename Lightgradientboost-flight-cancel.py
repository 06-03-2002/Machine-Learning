# -*- coding: utf-8 -*-
"""
Created on Thu Sep  4 18:37:47 2025
...
model_folder_name = "lgbm_trained_model_no_graphviz"
random_state = 42
test_size = 0.1
...
lgbm = LGBMClassifier(
    n_estimators=200,
    learning_rate=0.1,
    max_depth=-1,  # no max depth by default
    subsample=0.8,
...
if hasattr(lgbm, "predict_proba"):
    ...
    plt.plot(fpr, tpr, lw=2, label=f"LightGBM ROC (AUC={roc_auc:.3f})")
...
if hasattr(lgbm, "feature_importances_"):
    fi = pd.Series(lgbm.feature_importances_, index=features.columns).sort_values(ascending=True)
...
# ------------------------------
# Surrogate Decision Tree (to approximate LightGBM)
# ------------------------------
...
surrogate = DecisionTreeClassifier(max_depth=SURROGATE_MAX_DEPTH,
                                   ...
                                   random_state=random_state)
surrogate.fit(x_train, lgbm_train_pred)

surrogate_pred_on_test = surrogate.predict(x_test)
lgbm_pred_on_test = lgbm.predict(x_test)
fidelity = (surrogate_pred_on_test == lgbm_pred_on_test).mean()
print(f"\n🔁 Surrogate fidelity (matches LightGBM on test): {fidelity:.3f}")
...
plt.title(f"Surrogate Decision Tree (max_depth={SURROGATE_MAX_DEPTH}) — approximates LightGBM", fontsize=16)
plt.tight_layout()
plt.show()
...
lgbm_model_file = os.path.join(model_dir, "lgbm_cancel_model.pkl")
...
print("\n📁 Saved:")
print(f" - LightGBM model: {lgbm_model_file}")
