# -*- coding: utf-8 -*-
"""
Created on Wed Sep 17 02:05:27 2025
...
models_dir = os.path.join(os.path.dirname(data_path), "model-lgbm2")
...
for column_index, column in enumerate(features.columns):
    ...
    y_curve = lgbm.predict(X_curve)

    plt.plot(np.linspace(min(X_col), max(X_col), 200), y_curve,
             color="red", linewidth=2, label="LightGBM Fit")
...
for column_index, column in enumerate(features.columns):
    ...
    y_curve_raw = lgbm.predict(X_curve_scaled)
    ...
    plt.plot(grid_raw, y_curve_raw, color="red", linewidth=2, label="LGBM Fit (raw, imputed)")
